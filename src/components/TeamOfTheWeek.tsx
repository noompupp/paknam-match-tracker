  ...
  return (
    <div className="gradient-bg min-h-screen">
      ...
      <div className="max-w-2xl mx-auto py-6 px-2 sm:px-6 flex flex-col gap-8">
        ...
        {data?.length === 0 && (
          <p className="text-center text-muted-foreground">{t("rating.noMatches") || "No completed matches with events found."}</p>
        )}
        {data?.map((fixture) => (
                ...
                <div className="mb-2">
                  ...
                  <ul className="ml-4 mt-1 list-disc text-muted-foreground text-sm">
                    {(fixture.goalEvents || []).map((event: any, idx: number) => (
                      <li key={idx}>
                        <span className="font-medium">{event.player_name}</span> ({event.team_id}), <span className="text-xs">{event.event_type === "goal" ? "Goal" : "Assist"}</span>
                        <StarRating

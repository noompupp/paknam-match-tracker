...
>(({ className, children, hideCloseButton = false, ...props }, ref) => (
  <EnhancedDialogPortal>
    ...
    >
      {children}
      {!hideCloseButton && (
        <DialogPrimitive.Close className="absolute z-10 rounded-full opacity-80 ring-offset-background transition-all hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground bg-background/90 backdrop-blur-sm border shadow-sm hover:bg-accent/50 right-3 top-3 sm:right-4 sm:top-4 w-8 h-8 sm:w-9 sm:h-9 flex items-center justify-center">

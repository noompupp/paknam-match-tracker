  ...
  <EnhancedDialogPortal>
    ...
    <DialogPrimitive.Content
      ...
      className={cn(
        ...
        // Safe area support for mobile fullscreen
        "safe-all md:safe-none",
        className
      )}
      {...props}
    >
      {children}
      {!hideCloseButton && (
        <DialogPrimitive.Close className="absolute z-10 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground modal-close-safe">
          <X className="h-4 w-4" />
...
const EnhancedDialogHeader = ({
  ...
  <div
    className={cn(
      "flex flex-col space-y-1.5 text-center sm:text-left modal-safe-top",
      className
    )}
    {...props}
  />

...
import { Target, Trophy, Clock, Award, Users, ChevronDown, ChevronUp } from "lucide-react";
...
const MobileOptimizedPlayerCard = ({ 
  ...
  // Mobile portrait ultra-compact mode
  if (isMobile && isPortrait && variant === 'compact') {
    return (
      <div className="flex items-center justify-between rounded-lg border border-border/30 transition-all duration-200 hover:border-border/50 hover:bg-muted/30 group bg-background/60 backdrop-blur-sm p-2">
        <div className="flex items-center gap-2 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1">
            <h3 className="font-medium text-foreground truncate text-xs leading-tight">
        ...
        <div className="flex items-center gap-0.5 flex-shrink-0 ml-1">
          {hasStats ? (
            <>
              <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200 text-[10px] px-1 py-0 h-4 min-w-[16px] justify-center">
                {player.goals || 0}
              </Badge>
              <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 text-[10px] px-1 py-0 h-4 min-w-[16px] justify-center">
                {player.assists || 0}
              </Badge>
            </>
          ) : (
            <Badge variant="outline" className="text-muted-foreground text-[10px] px-1 py-0 h-4">
              -
            </Badge>
          )}
        </div>

...
const TeamLogoDisplay = ({ 
  ...
  const sizeClasses = {
    sm: isPremierLeagueStyle ? 'h-12 w-12' : 'h-10 w-10',
  ...
  const textSizeClasses = {
    sm: isPremierLeagueStyle ? 'text-xs md:text-sm' : 'text-sm',
  ...
  const fallbackTextSize = {
    sm: isPremierLeagueStyle ? 'text-sm md:text-base' : 'text-xs',
  ...
  return (
    <div className={`flex flex-col items-center ${gapClasses[size]}`}>
      ...
      {showName && (
        <div 
          className={`text-center px-2 md:px-4 py-1 md:py-2 rounded-lg ${isPremierLeagueStyle ? 'font-bold text-white' : ''}`}
          style={isPremierLeagueStyle ? {

-- Fix team name resolution in rating generation function
...
BEGIN
  ...
  -- Determine match results based on actual scores
  ...
  FOR player_record IN
    SELECT DISTINCT 
      ...
      ptt.player_name,
      ptt.team_id,
      -- FIXED: Properly resolve team names
      COALESCE(
        CASE 
          ...
          ELSE (SELECT t.name FROM teams t WHERE t.__id__ = ptt.team_id::text LIMIT 1)
        END,
        ptt.team_id::text
      ) as team_name,
      ptt.total_minutes,
      COALESCE(m."position", 'Player') as player_position,
      -- Use proper match result based on team
      CASE 
  ...
  LOOP
    ...
    clean_sheet_achieved := CASE 
      WHEN player_record.player_position IN ('GK', 'DF') THEN
        (player_record.team_id::text = fixture_data.home_team_id AND fixture_data.away_score = 0) OR
        (player_record.team_id::text = fixture_data.away_team_id AND fixture_data.home_score = 0)
      ELSE false
    END;
    ...
    -- Enhanced participation rating based on minutes
    participation_rating_value := CASE 
      WHEN player_record.total_minutes >= 45 THEN 8.0 + (RANDOM() * 1.5)
      WHEN player_record.total_minutes >= 35 THEN 7.5 + (RANDOM() * 1.0)
      WHEN player_record.total_minutes >= 25 THEN 7.0 + (RANDOM() * 0.8)
      WHEN player_record.total_minutes >= 15 THEN 6.5 + (RANDOM() * 0.5)
      ELSE 6.0 + (RANDOM() * 0.3)
    END;
    ...
    -- Insert/update the rating record with proper team_id casting
      ...
      jsonb_build_object(
        ...
        CASE 
          WHEN player_record.player_position = 'GK' THEN
            CASE 
              WHEN player_record.team_id::text = fixture_data.home_team_id THEN fixture_data.away_score
